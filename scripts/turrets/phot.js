const photonlaser = extend(BasicBulletType, {
	...
	update: function(b){
		...
		if(b.timer.get(1, 5)){
			Damage.collideLine(b, b.team, this.hitEffect, b.x, b.y, b.vel.angle(), 150.0, false);
		}
	},
	...
	draw: function(b){
		...
		for(var s = 0; s < 3; s++){
			...
			for(var i = 0; i < 4; i++){
				...
				Lines.lineAngle(b.x + Tmp.v1.x, b.y + Tmp.v1.y, b.vel.angle(), 147.0 * b.fout() * lenscales[i], false);
			}
		};
		Draw.reset();
	}
