...
const refractorLaser = extend(BasicBulletType, {
	...
	update: function(b){
		...
		if(b.timer.get(1, 5)){
			Damage.collideLine(b, b.team, this.hitEffect, b.x, b.y, b.vel.angle(), this.laserLength * 1.10, true);
		};
		trnsB.trns(b.vel.angle(), Mathf.random(0.5, 270.0), Mathf.range(8.0));
		Effect.effect(refractorLaserEffect, b.x + trnsB.x, b.y + trnsB.y, b.vel.angle());
	},
	...
	draw: function(b){
		...
		for(var s = 0; s < 6; s++){
			...
			for(var i = 0; i < 4; i++){
				Tmp.v1.trns(b.vel.angle() + 180.0, (lenscales[i] - 1.0) * 34.0);
				Lines.stroke((8 + Mathf.absin(Time.time + (2 * s), 1.9, 1.3)) * b.fout() * strokes[s] * tscales[i]);
				Lines.lineAngle(b.x + Tmp.v1.x, b.y + Tmp.v1.y, b.vel.angle(), this.laserLength * b.fout() * lenscales[i], false);
			}
		};
		Draw.reset();
	}
